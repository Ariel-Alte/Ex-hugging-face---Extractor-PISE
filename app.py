...
def extraer_clasificaciones(paginas: list) -> dict:
    """
    Lee D. Conclusiones usando regex directo.
...
# ══════════════════════════════════════════════════════════
#  PARSERS POR SECCIÓN (igual al original)
# ══════════════════════════════════════════════════════════

def _detectar_todos_headers_ruedas(tabla):
    """
    Devuelve lista de (header_idx, col_map) para todos los bloques RUEDA.
    """
    bloques = []
    i = 0
    while i < len(tabla):
...
def parse_ruedas(pagina: dict, enc: dict, seccion: str) -> list:
    """Extrae TODOS los valores numéricos de ruedas."""
    filas = []
...
def parse_frenos_neumatica(pagina: dict, enc: dict, seccion: str) -> list:
    """TEÓRICO | REAL | OBS (locomotora) o CONTROL EN CABINA | OBS (coche)."""
    filas = []
    obs_txt = extraer_obs_texto(pagina["texto"])
    for tabla in pagina["tablas"]:
        if not tabla or len(tabla) < 3:
...
def parse_traccion(pagina: dict, enc: dict, seccion: str) -> list:
    """OK | OBSERVACION con columnas D/I. Extrae X y valores numéricos."""
    filas = []
    obs_txt = extraer_obs_texto(pagina["texto"])
    for tabla in pagina["tablas"]:
        if not tabla or len(tabla) < 3:
...
def parse_frenos_mecanica_multirueda(pagina: dict, enc: dict, seccion: str) -> list:
    """Parser para sección 3 de PISE-024: tabla de 21 cols."""
    filas = []
    obs_txt = extraer_obs_texto(pagina["texto"])
    for tabla in pagina["tablas"]:
        if not tabla or len(tabla) < 5 or len(tabla[0]) < 10:
            continue
...
def main():
    st.set_page_config(page_title="PISE Extractor", layout="wide", initial_sidebar_state="expanded")
    ...
    **Bureau Veritas Argentina**
    ...
    with col2:
        ...
        if st.button("Procesar PDFs", use_container_width=True, type="primary"):
            if not uploaded_files:
                st.warning("⚠️ Por favor cargá al menos un archivo PDF")
            else:
                    ...
                    try:
                        ...
                        with st.spinner(f"Procesando {uploaded_file.name}..."):
                            ...
                            resultado_txt = (
                                ...
                                f"🔴 Críticas: {nc} | 🟡 Normales: {nn} | "
                                f"🟢 Reparadas: {nr} | ⚫ Rechazadas: {nrec}"
                            )
                            resultados.append(resultado_txt)
                ...
                if todas_filas:
                    ...
                    st.markdown(f"""
                    ---
                    **Total registros:** {n_tot}  
                    **Con observación:** {n_obs}
                    """)
                    ...
                    st.download_button(
                        ...
                        data=excel_buf,
                        file_name="PISE_Extraccion.xlsx",
                        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                        use_container_width=True
                    )
                else:
                    st.error("⚠️ No se extrajeron datos de los archivos.")
    
    st.divider()
    st.markdown("""
    ---
    **Secciones soportadas:**  
    1 Ruedas · 2a/2b Bogies · 2c Mesa Central · 3a Frenos Mecánica · 3b Frenos Neumática · 
    4 Tracción y Choque · 5 Bajo Bastidor · 6 Interior · 7 Estructura · 8 Motores · 9 Cabina · 10 ATS

    **PISE compatibles:**  
    PISE-SGBV-030 (Coches DMU) · PISE-SGBV-011 (Locomotoras GM)
    """)
